...
exports.generateRequirements = exports.getFileContent = exports.deleteProject = exports.debugProjectFiles = exports.debugProjects = exports.getExtractedFiles = exports.getProject = exports.getProjects = exports.uploadProject = void 0;
...
const debugProjectFiles = async (req, res) => {
    try {
        const projectId = parseInt(req.params.id);
        const userId = req.user.id;
        // Verify project ownership
        ...
        const countResult = await database_1.default.query('SELECT COUNT(*) FROM extracted_files WHERE project_id = $1', [projectId]);
        const sampleFiles = await database_1.default.query('SELECT file_path, file_name, file_type, is_directory FROM extracted_files WHERE project_id = $1 ORDER BY file_path LIMIT 20', [projectId]);
        res.json({
            projectId,
...
exports.debugProjectFiles = debugProjectFiles;
...
const getFileContent = async (req, res) => {
    try {
        ...
        const file = await ExtractedFile_1.ExtractedFileModel.findByProjectIdAndPath(projectId, filePath);
        if (!file) {
...
const generateRequirements = async (req, res) => {
    try {
        const projectId = parseInt(req.params.id);
        const userId = req.user.id;
        // Verify project ownership
        ...
        const allFiles = await ExtractedFile_1.ExtractedFileModel.findAllByProjectId(projectId);
...
async function analyzeAndGenerateRequirements(project, files) {
    const analysis = {
...
function extractApplicationName(project, files) {
    // Try to extract from project name first
    let appName = project.name;
    // Look for application.xml or similar files
    const appFiles = files.filter(f => f.file_name.toLowerCase().includes('application') &&
        f.file_type === '.xml');
...
function extractDataModels(files) {
    ...
    const classFiles = files.filter(f => f.file_name.toLowerCase().includes('class') &&
        f.file_type === '.xml');
    const propertyFiles = files.filter(f => f.file_name.toLowerCase().includes('property') &&
        f.file_type === '.xml');
...
function extractWorkflows(files) {
    const workflows = [];
    // Look for flow files, processes, activities
    const flowFiles = files.filter(f => (f.file_name.toLowerCase().includes('flow') ||
...
function extractRules(files) {
    const rules = [];
    // Look for rule files
    const ruleFiles = files.filter(f => f.file_name.toLowerCase().includes('rule') &&
        f.file_type === '.xml');
...
function extractIntegrations(files) {
    const integrations = [];
    // Look for connector files, service definitions
    const connectorFiles = files.filter(f => (f.file_name.toLowerCase().includes('connector') ||
...
function extractUIComponents(files) {
    const uiComponents = [];
    // Look for section files, harness files, flow actions
    const uiFiles = files.filter(f => (f.file_name.toLowerCase().includes('section') ||
...
function extractConfigurations(files) {
    const configurations = [];
    // Look for configuration files, properties files
    const configFiles = files.filter(f => f.file_type === '.properties' ||
...
function extractFlowName(content) {
    const match = content.match(/<Flow[^>]*name="([^"]+)"/i);
    return match ? match[1] : null;
}
function extractFlowSteps(content) {
    const steps = [];
...
function extractRuleName(content) {
    const match = content.match(/<Rule[^>]*name="([^"]+)"/i);
    return match ? match[1] : null;
}
function extractRuleType(content) {
    const match = content.match(/<Rule[^>]*type="([^"]+)"/i);
    return match ? match[1] : null;
}
function extractConnectorName(content) {
    const match = content.match(/<Connector[^>]*name="([^"]+)"/i);
    return match ? match[1] : null;
}
function extractConnectorType(content) {
...
function extractComponentType(content) {
    const match = content.match(/<(Section|Harness|FlowAction)[^>]*/i);
    return match ? match[1] : null;
}
function generateRequirementsDocument(analysis) {
...
## 6. UI Components
${uiComponents.length > 0 ? uiComponents.map((component) => `
### ${component.name}
...
async function processEntriesRecursively(projectId, entries, basePath, processedFiles) {
            ...
            if (dirPath) {
                ...
                try {
                    await ExtractedFile_1.ExtractedFileModel.create({
                        ...
                        is_directory: true,
                        parent_directory: path_1.default.dirname(dirPath) === '.' ? (basePath || undefined) : (basePath ? `${basePath}/${path_1.default.dirname(dirPath)}` : path_1.default.dirname(dirPath))
                    });
                    processedFiles++;
                }
                catch (dbError) {
